...
// src/providers/nft-collections.ts
var createNftCollectionProvider = (nftService, marketIntelligenceService, socialAnalyticsService) => {
  return {
    get: async (runtime, message) => {
      ...
      for (const collection2 of collections) {
        response += `${collection2.name}:
`;
        response += `\u2022 Floor Price: ${collection2.floorPrice} ETH
`;
        response += `\u2022 24h Volume: ${collection2.volume24h} ETH
`;
        response += `\u2022 Market Cap: ${collection2.marketCap} ETH
`;
        response += `\u2022 Holders: ${collection2.holders}

`;
      }
      const collection = collections.find(
        (c) => message.content.text.toLowerCase().includes(c.name.toLowerCase()) || message.content.text.toLowerCase().includes(c.address.toLowerCase())
      );
      if (collection) {
        response += `
...
// src/actions/get-collections.ts
var getCollectionsAction = (nftCollectionProvider) => {
  return {
    name: "GET_NFT_COLLECTIONS",
    similes: ["LIST_NFT_COLLECTIONS", "SHOW_NFT_COLLECTIONS"],
    description: "Fetches information about curated NFT collections on Ethereum",
    ...
    handler: async (runtime, message, _state, _options, callback) => {
      try {
        ...
        const errorMessage = error instanceof Error ? `Failed to fetch NFT collections: ${error.message}` : "An unexpected error occurred while fetching NFT collections.";
...
function extractListingDetails(text) {
  ...
  return {
    ...
    price: priceMatch ? Number.parseFloat(priceMatch[1]) : void 0
  };
}
var listNFTAction = (nftService) => {
  return {
    name: "LIST_NFT",
    similes: ["SELL_NFT", "CREATE_LISTING"],
    description: "Lists an NFT for sale on ikigailabs.xyz marketplace at double the purchase price.",
    validate: async (_runtime, message) => {
      const content = message.content.text.toLowerCase();
      return (content.includes("list") || content.includes("sell")) && content.includes("nft") && (content.includes("0x") || content.includes("token") || content.includes("#"));
    },
    handler: async (runtime, message, _state, _options, callback) => {
      try {
        const {
        ...
        const ownedNFT = ownedNFTs.find(
          (nft) => nft.collectionAddress.toLowerCase() === collectionAddress.toLowerCase() && nft.tokenId === tokenId
        );
        if (!ownedNFT) {
        ...
        const listing = await nftService.createListing({
          tokenId,
          collectionAddress,
          price: userSpecifiedPrice || 0,
          // Default to 0 if no price specified
          marketplace: "ikigailabs",
          expirationTime: Math.floor(Date.now() / 1e3) + 30 * 24 * 60 * 60
          // 30 days
        });
...
// src/actions/sweep-floor.ts
function extractNFTDetails(text) {
...
var sweepFloorAction = (nftService) => {
  return {
    name: "SWEEP_FLOOR_NFT",
    similes: ["BUY_FLOOR_NFT", "PURCHASE_FLOOR_NFT"],
    description: "Sweeps the floor of a specified EVM NFT collection by purchasing the lowest-priced available NFTs.",
    validate: async (_runtime, message) => {
      const content = message.content.text.toLowerCase();
      return (content.includes("sweep") || content.includes("buy")) && content.includes("nft") && (content.includes("0x") || content.includes("floor"));
    },
    handler: async (runtime, message, _state, _options, callback) => {
      try {
        ...
        const result = await nftService.executeBuy({
          ...
          // Assuming userId is the wallet address
        });
        const totalPrice = floorListings.reduce(
          (sum, listing) => sum + listing.price,
          0
        );
        const response = `Successfully initiated sweep of ${quantity} NFTs from collection ${collectionAddress}:
...
// src/services/reservoir.ts
var import_p_retry = __toESM(require_p_retry(), 1);

// src/utils/performance.ts
import { EventEmitter } from "node:events";
var PerformanceMonitor = class _PerformanceMonitor extends EventEmitter {
  static instance;
  metrics = [];
  maxMetrics = 1e3;
  alertThresholds = {
  ...
  getAverageLatency(operation, timeWindowMs = 6e4) {
  ...
  getErrorRate(operation, timeWindowMs = 6e4) {
    const relevantMetrics = this.getRecentMetrics(operation, timeWindowMs);
    if (relevantMetrics.length === 0) return 0;
  ...
  getThroughput(operation, timeWindowMs = 6e4) {
    const relevantMetrics = this.getRecentMetrics(operation, timeWindowMs);
    return relevantMetrics.length / timeWindowMs * 1e3;
  }
  // Get performance summary
  getPerformanceSummary(timeWindowMs = 6e4) {
  ...
  getRecentMetrics(operation, timeWindowMs) {
    const now = /* @__PURE__ */ new Date();
    const windowStart = new Date(now.getTime() - timeWindowMs);
    return this.metrics.filter(
      (metric) => metric.operation === operation && metric.timestamp >= windowStart
    );
  }
...
var ErrorCode = /* @__PURE__ */ ((ErrorCode2) => {
  ...
  ErrorCode2["INVALID_TOKEN_ID"] = "INVALID_TOKEN_ID";
  ErrorCode2["INVALID_PRICE"] = "INVALID_PRICE";
  ErrorCode2["INVALID_DATA"] = "INVALID_DATA";
  ...
  ErrorCode2["API_RESPONSE_INVALID"] = "API_RESPONSE_INVALID";
...
var ErrorHandler = class _ErrorHandler {
  ...
  handleAPIError(error) {
    if (error.code === "API_KEY_INVALID" /* API_KEY_INVALID */) {
...
var ReservoirService = class {
  ...
  errorHandler;
  constructor(config2 = {}) {
    this.cacheManager = config2.cacheManager;
    this.rateLimiter = config2.rateLimiter;
    this.maxRetries = config2.maxRetries || 3;
    this.batchSize = config2.batchSize || 20;
    this.performanceMonitor = PerformanceMonitor.getInstance();
    this.errorHandler = ErrorHandler.getInstance();
  }
  async makeRequest(endpoint, params = {}, priority = 0, runtime) {
  ...
  async getTopCollections(runtime, limit = 10) {
    const endOperation = this.performanceMonitor.startOperation(
      "getTopCollections",
      { limit }
    );
  ...
  async getCollectionActivity(_collectionAddress) {
    return Promise.resolve(null);
  }
  async getCollectionTokens(_collectionAddress) {
    return Promise.resolve(null);
  }
  async getCollectionAttributes(_collectionAddress) {
    return Promise.resolve(null);
  }
...
var RateLimiter = class {
  ...
  constructor(config2 = {}) {
    this.limiter = new RateLimiterMemory({
      points: config2.maxRequests || 100,
      duration: (config2.windowMs || 6e4) / 1e3
  ...
  async consume(key, points = 1) {
    try {
      ...
      if (error instanceof Error && "remainingPoints" in error) {
        ...
        const retryAfter = Math.ceil(rateLimitError.msBeforeNext / 1e3);
        throw new Error(
    ...
    while (retries <= this.maxRetries) {
      try {
        ...
        if (error instanceof Error && ((_a2 = error.message) == null ? void 0 : _a2.includes("Rate limit exceeded"))) {
          ...
          await new Promise(
            (resolve) => setTimeout(resolve, this.retryDelay * retries)
          );
        } else {
          break;
        }
...
// src/services/market-intelligence.ts
var MarketIntelligenceService = class {
  cacheManager;
  rateLimiter;
  constructor(config2 = {}) {
    this.cacheManager = config2.cacheManager;
  ...
  async getMarketIntelligence(_address) {
    return {
      floorPrice: 0,
      volume24h: 0,
      marketCap: 0,
...
// src/services/social-analytics.ts
var SocialAnalyticsService = class {
  cacheManager;
  rateLimiter;
  constructor(config2 = {}) {
    this.cacheManager = config2.cacheManager;
  ...
  async getSocialMetrics(_address) {
    return {
      lastUpdate: (/* @__PURE__ */ new Date()).toISOString()
    };
  }
  async getCommunityMetrics(_address, _discordId, _telegramId) {
    return {
      lastUpdate: (/* @__PURE__ */ new Date()).toISOString()
    };
  }
  async analyzeSentiment(_address) {
    return {
      overall: 0,
      breakdown: {
        positive: 0,
  ...
  async trackSocialPerformance(_address) {
    return {
      metrics: {
        reach: 0,
        engagement: 0,
...
var index_default = createNFTCollectionsPlugin;
export {
